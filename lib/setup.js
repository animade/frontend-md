...
function correct_object_syntax() {
  ...
  c.log('\t\t"templates": "PATH_TO_YOUR_TEMPLATES_FOLDER",');
  c.log('\t\t"stylesheets": "PATH_TO_YOUR_JAVASCRIPTS_FOLDER",');
  c.log('\t\t"javascripts": "PATH_TO_YOUR_STYLESHEETS_FOLDER"');
  c.log('\t}');
  c.log('}');
}
...
var self = module.exports = {
  ...
  init: function init(callback) {
    ...
    if (fs.existsSync(config_path)) {
      ...
      } else if (!fs.existsSync(n.get('frontend').templates)) {

        // ------------------------------------------------
        // Check for Templates folder accesibility
        //

        c.error("Couldn't find templates folder \"" + n.get('frontend').templates + "\"");
