...
var ignore_extensions = ["DS_Store", "keep"];
var folder_history = [];
var indentation_target = 40;
  ...
  walker.on('file', function(root, stat, next) {
    ...
    fs.readFile(full_path, 'utf8', function(err, data) {
      ...
      if (err) {
        ...
        var ext = _.last(full_path.split('.'));

        // Add this file to the list of files
        if (ignore_extensions.indexOf(ext) < 0) {
...
function parse_comments(file, ext, spaces_added) {
  ...
  if ((ext == 'sass') || (ext == 'scss') || (ext == 'jade') || (ext == 'js') || (ext == 'less')) {
    ...
    if (lines.length > 0) {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (comment_starters.indexOf(lines[i][0]) > -1) {
          ...
          // ------------------------------------------------
          // Jade specific: check if this is a multiline comment and jump to next line
          //
          ...
          var formatted_comment = lines[i]
            ...
            .replace(/\*/, "") // Replace comments - *
            .replace(/(^\/\/\-+\n\s]+)/g, '') // Replace comments - Multiline //-
...
function iterate_asset_folders(arr, callback) {
  ...
  render_code_folder_structure(arr[asset_type_counter].path, arr[asset_type_counter].title, function() {
    ...
    // Reset the folder history between goes
    folder_history = [];
